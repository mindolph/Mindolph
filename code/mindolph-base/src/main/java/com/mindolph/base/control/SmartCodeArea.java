...
public class SmartCodeArea extends ExtCodeArea implements Anchorable {
                        ...
                        Platform.runLater(() -> {
                            if (!streamOutput.streamPartial().isStop()) {
                                if (StringUtils.isNotBlank(SmartCodeArea.this.getSelectedText())) {
                                    ...
                                    SmartCodeArea.this.insertText(streamOutput.streamPartial().text());
                                }
                                if (log.isTraceEnabled()) log.trace(String.valueOf(streamOutput.streamPartial()));
    ...
    private Point2D getPanelTargetPoint() {
        ...
        // NOTE: since the v0.11.5 the getCharacterBoundsOnScreen(0, 0) throws exception when the first paragraph is invisible,
        // so instead, get the minX from the first visible bounds on screen for workaround, the visible bounds is a litter bit different from the character bounds for the gutter is included.
        Bounds visibleParagraphBoundsOnScreen = super.getFirstVisibleBoundsOnScreen();
...
//        Bounds leftSideBoundsInScreen = optBounds.orElse(BoundsUtils.newZero());
        ...
        Point2D targetPointInScreen = new Point2D(visibleParagraphBoundsOnScreen.getMinX(), caretBoundsInScreen.getMaxY());
