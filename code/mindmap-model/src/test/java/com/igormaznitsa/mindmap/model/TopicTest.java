...
//  };
  ...
  public void testFindMaxChildPathLength() {
    final MindMap<TestTopicNode> map = createMindMapWithRoot();
  ...
  @Test
  public void test() {
    final MindMap mm = createMindMapWithRoot();
  ...
  public void testParse_OnlyTopic() throws Exception {
    final MindMap mm = createMindMapWithRoot();
    final Topic topic = Topic.parse(mm, makeLexer("# Topic"), TestTopicNode.testTopicCreator);
  ...
  public void testParse_OnlyTopicWithExtras() throws Exception {
    final MindMap<TestTopicNode> mm = createMindMapWithRoot();
    final TestTopicNode topic = Topic.parse(mm, makeLexer(
  ...
  public void testParse_PairTopicsFirstContainsMultilineTextNoteAndMiscEOL() throws Exception {
    final MindMap mm = createMindMapWithRoot();
    final TestTopicNode topic = (TestTopicNode) TestTopicNode.parse(mm, makeLexer(
        "# Topic\r\n- NOTE\r\n<pre>   Some   \r\n    text     \n    line  \r\n  end \r\n   </pre>\r\n- LINK\n<pre>http://www.google.com</pre>\n## Topic2"), TestTopicNode.testTopicCreator);
    assertEquals("Topic", topic.getText());
    assertEquals(1, topic.getChildren().size());
    assertEquals(2, topic.getExtras().size());
  ...
  public void testParse_TopicAndChild() throws Exception {
    final MindMap mm = createMindMapWithRoot();
    final TestTopicNode topic = (TestTopicNode) TestTopicNode.parse(mm, makeLexer("# Topic<br>Root\n ## Child<br/>Topic"), TestTopicNode.testTopicCreator);
    assertEquals("Topic\nRoot", topic.getText());
    assertEquals(1, topic.getChildren().size());
  ...
  public void testParseRoot_CodeSnippetContainsThreeBackticksLine_withSpacesBefore()
      throws Exception {
    final MindMap mm = createMindMapWithRoot();
    final TestTopicNode topic = (TestTopicNode) TestTopicNode.parse(mm, makeLexer("# Root\n```Java\nSystem.exit(0);\n ```\n```"), TestTopicNode.testTopicCreator);
  ...
  public void testParse_TopicAndTwoChildren() throws Exception {
    final MindMap mm = createMindMapWithRoot();
    final TestTopicNode topic = (TestTopicNode) TestTopicNode.parse(mm, makeLexer("# Topic\n ## Child1\n ## Child2\n"), TestTopicNode.testTopicCreator);
    assertEquals("Topic", topic.getText());
  ...
  public void testParse_MultiLevels() throws Exception {
    final MindMap mm = createMindMapWithRoot();
    final TestTopicNode root = (TestTopicNode) TestTopicNode.parse(mm, makeLexer(
    ...
    final TestTopicNode level32 = root.getChildren().get(1).getChildren().get(0);
  ...
  public void testParse_topicWithTextStartsWithHash() throws Exception {
    ...
    final MindMap parsed = new MindMap(new StringReader(packedMap), TestTopicNode.testTopicCreator);
  ...
  public void testWriteParse_encryptedNote() throws Exception {
    ...
    final MindMap parsed = new MindMap(new StringReader(written), TestTopicNode.testTopicCreator);
  ...
  public void testParse_noteContainsTicks() throws Exception {
    ...
    final MindMap parsed = new MindMap(new StringReader(text), TestTopicNode.testTopicCreator);
